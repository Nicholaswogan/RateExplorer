...
module fractal_meanfield_mod
  ...
  !!
  !! Generate optical properties for fractal particles.
  !!
  !! See Botet et al. 1997 "Mean-field approximation of Mie
  !! scattering by fractal aggregates of identical spheres."
  !! Applied Optics 36(33) 8791-8797
  ...
  !! @author  P.Rannou, R.Botet, Eric Wolf
  !! @version March 2013
  subroutine fractal_meanfield(do_miess_in, xl_in, xk_in, xn_in, nb_in, alpha_in, &
    df_in, rmon,xv, ang, rcore, xksh_in, xnsh_in, Qext, Qsca, gfac, rc)

    logical, intent(in)                :: do_miess_in   !! do core shell mie calculation for monomers
    real(kind=f),intent(in)            :: xl_in         !! Wavelength [microns]
    real(kind=f),intent(in)            :: xk_in         !! imaginary index of refraction ! if do_miess_in = .true, core imaginary index
    real(kind=f),intent(in)            :: xn_in         !! real index of refraction      ! if do_miess_in = .true, core real index
    ...
    real(kind=f),intent(in)            :: rcore         !! core radius [microns]               ! only used if do_miess_in = .true.  
    real(kind=f),intent(in)            :: xksh_in       !! shell imaginary index of refraction ! only used if do_miess_in = .true.  
    real(kind=f),intent(in)            :: xnsh_in       !! shell real index of refraction      ! only used if do_miess_in = .true.  
    ...
    real(kind=f)                       :: xnsh(39)        ! place holder for real index of refraction
    real(kind=f)                       :: xksh(39)        ! place holder for imaginary index of refraction
    real(kind=f)                       :: val           
    ...
    complex(kind=f)                    :: ni,nish,nico,i,id,onec,zeroc
    ...
    xl(1) = xl_in
    xk(1) = xk_in   ! monomer/core refractive index
    xn(1) = xn_in    
    xksh(1) = xksh_in  ! shell refractive index
    xnsh(1) = xnsh_in    
    ...
    funca(:,:,:) = 0.0_f
    fx_vars%a = rmon*1.e-6_f            ! a = rmonomer in m
    fx_vars%ac = rcore*1.e-6_f          ! ac = rcore in m
    ...
    fx_vars%coeff=anorm(fx_vars,rc)
    if (rc < 0) return
    ...
    kk=1 
    ni=xn(kk)*1._f+i*xk(kk)*xv*1._f        ! ni := complex index of refraction of monomer/core => cmie 
    nico=xn(kk)*1._f-i*xk(kk)*xv*1._f      ! nico := complex index of refraction of core => miess
    nish=xnsh(kk)*1._f-i*xksh(kk)*xv*1._f  ! nish := complex index of refraction of shell => miess
    ...
    if (do_miess_in) then 
      ! call core shell mie
      call miess(fx_vars%ac, fx_vars%a, fx_vars%k, nish, nico, CosAng, 0, 1, an, bn, nstop)
    else
      ! call regular mie for monomer particle
    ...
    if (n2stop.gt.48) then
      write(*, *) "fractal_meanfield_mod::n2stop greater than 48. Size parameter (2*pi*rmon/lambda): ",2._f*3.14159265_f*fx_vars%a/lbd, "Monomer Size parameter must be less than ~17."
      rc = RC_ERROR
      return
    endif
    ...
    do pp=0,n2stop       !variable p
      res = funs_n(fx_vars,pp,rc)   ! Eq.(16)    S_p(k R_g)
      if (rc < 0) return
      funcs(pp) = res
    end do
    ...
    do ii=1,n1stop       !variable n
      do jj=1,n1stop     !variable nu
        ...
        do pp=mm,tt
          ...
          funca(jj,ii,pp) = funa(fx_vars,jj,ii,pp,rc)       ! Eq.(14)    a(nu,n;p)a
          if (rc < 0) return
    ...
    do q=1,ntheta-2,2
      ...
      temp1=2._f/6._f*3.1415926353_f/(ntheta-1)*(a0+4._f*a1+a2)
      total=total+temp1 

      temp2=2._f/6._f*3.1415926353_f/(ntheta-1)*(a0*c0+4._f*a1*c1+a2*c2)
      totg=totg+temp2
!     write(*,*) angle, s11(q+1)
    end do
    totg=totg/total
...
300 continue
  ...
  !! @author P. Rannou, R. Botet, Eric Wolf
  !! @version March 2013
  function funa(fx_vars,nu,n,p,rc)
  ...
  function funs_n(fx_vars,pp,rc)
    ...
    if (ifail.ne.0) then
      ! write(*,*) "funs_n::ifail=",ifail," returned by dqagi() during call of funs(",pp,") while integrating xfreal_n()"
      ! rc = RC_ERROR
      ! return
    endif
